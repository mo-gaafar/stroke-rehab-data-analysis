...
class ModelTrainer:
    def train_model(self, model_data):  
        ...
        model_name=model_data['name'] if 'name' in model_data else (model_data['function'].__class__.__name__)
        ...
        model = cnn_stft_instance.create_model(data_shape, num_class, dropout, print_summary=True)
        logdir = 'logs'
        tensorboard_callback = TensorBoard(log_dir=logdir)

        history = model.fit(data['x_train'],data['y_train'],batch_size=64, epochs=num_epoch, validation_data=(data['x_val'],data['y_val']), callbacks=[tensorboard_callback])
...
class CNN_STFT:
    ...
    def create_model(self, input_shape, classes, dropout, print_summary=False):
        ...
        # Conv Block 1
        model.add(Conv2D(filters=128, kernel_size=(3, 3), input_shape=input_shape, padding='same'))
